...
import { ref, query, orderByChild, limitToLast, get, runTransaction } from "firebase/database";
...
export const saveScore = async (uid: string, displayName: string, score: number) => {
  ...
  // 1. Save to local storage (Unique logic handled inside)
  saveLocalScore(entry);

  // 2. Try saving to Firebase (Transaction to ensure Max Score)
  try {
    const userScoreRef = ref(db, `${DB_PATH}/${uid}`);
    
    await runTransaction(userScoreRef, (currentData) => {
      if (currentData === null) {
        return entry; // Create if doesn't exist
      }
      ...
      // Abort transaction if score is lower (return undefined)
      return;
    });

  } catch (error: any) {
    console.warn("Firebase save failed (using local only):", error.message);
  }
};
